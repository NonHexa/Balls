...
function startBossFight() {
    ...
    else if (bossType === 4) gameState.enemies.push(new (class extends BossEnemy { constructor(){super(gameState.center.x+200,gameState.center.y);this.size=35;this.life=12+gameState.bossLevel*2;this.color=palettes[gameState.currentPalette].bossColors[3];this.orbitAngle=0;this.orbitSpeed=0.015;this.orbitDist=250;this.spawnTimer=0;this.minionCount=0;this.maxMinionCount=8;this.maxGeneration=0;} update(dt){this.orbitAngle += this.orbitSpeed * dt; this.x = gameState.center.x + Math.cos(this.orbitAngle) * this.orbitDist; this.y = gameState.center.y + Math.sin(this.orbitAngle) * this.orbitDist; this.vx=0; this.vy=0; this.spawnTimer += dt; if (this.spawnTimer > 300 && this.minionCount < this.maxMinionCount) { this.spawnTimer = 0; this.minionCount++; gameState.enemies.push(new Enemy(enemyTypes.fast)); } this.minionCount = Math.max(0, this.minionCount - 0.003 * dt); if (this.life < 6) this.orbitSpeed = 0.025; } draw(ctx){ super.draw(ctx); ctx.strokeStyle='rgba(0,255,170,0.15)'; ctx.lineWidth=1; ctx.beginPath(); ctx.arc(gameState.center.x, gameState.center.y, this.orbitDist, 0, Math.PI*2); ctx.stroke(); }})());
